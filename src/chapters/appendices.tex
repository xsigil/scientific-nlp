...
\epigraph{"I use Arch BTW."\\
\small（ところで、私はArchを使っているのだが。）}{--- Anonymous Unix Hacker / The Arch Linux Creed}
%FIXME: とりあえずAIに執筆環境を書かせたので、全部書き直し。

本書の全体系を決定論的に記述・コンパイル・デプロイするために構築された、完全キーボード駆動型・超低遅延オーサリング環境の全仕様を開示する。
WYSIWYGエディタに特有の認知ノイズを排し、認知帯域（CEN）のすべてを概念の自己組織化に専有させるための工学的設計である。

\subsection{設計思想：認知摩擦の物理的排除}
...
\subsection{ツールチェーンとシステム構成}
本環境を構成するハードウェア・ソフトウェアの全ノードは、標準入出力（stdin/stdout）および決定論的IPCによって直結されている。
...
\begin{itemize}
    ...
    端末内のワークスペースを仮想化。編集バッファ（Vim）とビルド監視プロセス（\texttt{latexmk}）を左右に分割常駐させ、セッションの永続性を担保する。
\end{itemize}
