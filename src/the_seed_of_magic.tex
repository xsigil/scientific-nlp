...
\subsection{概要と問題設定}
本節では、著者が米国フロリダのNLP高額セミナーにおいて体験した「TIMEXの時計破損と壇上メタファーの完全符合」という実体験を題材とし、主観的に強烈な変性意識状態（$\mathcal{S}(\text{ALTER})$）をもたらす現象が、情報論的にいかにして発生したかを疑似ベイズ推論モデルおよびオセロ・フィルターに基づき冷徹に事後検証（ポストモーテム）する。
...
% 【論点】
%   1. 理論ベイズと神経疑似ベイズの乖離は「バグ」ではなく「非対称損失関数に対する最適化」
...
\subsection*{【補遺メモ】非対称損失関数と神経疑似ベイズの進化的妥当性}
...
\begin{itemize}
    \item \textbf{物理層（生命維持）:} 爆弾の落下、肉食獣の接近など、即死ペナルティが存在する空間では、理論ベイズの検算（ベースレートの精査）を待つ時間は致命的である。このため神経疑似ベイズ推論機は事前確率の項をバイパス（ハードワイヤード・シャント）し、直感（本能）を優先させる。
